...
//rellenar
	...
	float pixelFin = sqrtf(gradX*gradX+gradY*gradY);
...
int main(int argc, char **argv) {
  ...
  cudaMemcpy(device_image, host_image, width * height  * sizeof(unsigned char), cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(host_image, device_grayscale, width * height  * sizeof(unsigned char), cudaMemcpyDeviceToHost);

  // Show grayscale image
  cv::imshow("Grayscale Image", image);
  cv::waitKey(0);
  cv::imwrite("imagen_sobel.png", image);
